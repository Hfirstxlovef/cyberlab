<template>
  <div class="blue-team-achievement-list-page">
    <!-- 动态光晕背景层 -->
    ...
    <el-card class="page-card">
      ...
      <el-table
        ...
        v-loading="loading"
        style="width: 100%"
        :header-cell-style="{ background: 'rgba(30, 30, 30, 0.5)', color: 'rgba(255, 255, 255, 0.9)', fontWeight: '600', borderBottom: '1px solid rgba(0, 212, 255, 0.2)' }"
        :row-style="{ height: '60px' }"
        :cell-style="{ padding: '16px 12px', borderBottom: '1px solid rgba(0, 212, 255, 0.1)' }"
        empty-text="暂无提交记录">
        <el-table-column prop="id" label="ID" width="80" align="center"></el-table-column>
        <el-table-column prop="targetName" label="防御事件" width="200" show-overflow-tooltip></el-table-column>
    ...
    <el-dialog v-model="detailDialogVisible" title="成果详情" width="70%">
      <div v-if="currentAchievement" class="achievement-detail">
        <el-descriptions :column="2" border>
          <el-descriptions-item label="防守队名称">{{ currentAchievement.teamName }}</el-descriptions-item>
          <el-descriptions-item label="防御事件">{{ currentAchievement.targetName }}</el-descriptions-item>
          ...
          <el-descriptions-item label="防御工具/方法" :span="2">{{ currentAchievement.attackMethod || '未填写' }}</el-descriptions-item>
          <el-descriptions-item label="防御描述" :span="2">
...
const fetchMyAchievements = async (showMessage = false) => {
  try {
    ...
    try {
      ...
      // API未就绪，使用本地存储的提交记录
      const allSubmissions = JSON.parse(localStorage.getItem('blueTeamSubmissions') || '[]')
...
/* CSS Variables */
:root {
  --blue-team-primary: #00d4ff;
  --blue-team-secondary: #4dd0ff;
  --blue-team-dark-bg: rgba(10, 20, 40, 0.6);
  --blue-team-darker-bg: rgba(13, 26, 45, 0.6);
  --blue-team-border: rgba(0, 212, 255, 0.2);
  --radius-sm: 12px;
  --radius-md: 16px;
  --radius-lg: 20px;
  --spacing-xs: 8px;
...
.blue-team-achievement-list-page {
  background: linear-gradient(135deg, #0a1428 0%, #0d1a2d 50%, #1a2942 100%);
  min-height: 100vh;
...
/* 主卡片 - 深色毛玻璃 */
.page-card {
  background: var(--blue-team-dark-bg);
  backdrop-filter: blur(20px);
  -webkit-backdrop-filter: blur(20px);
  border: 0.5px solid var(--blue-team-border);
...
.page-card:hover {
  box-shadow: 0 12px 48px rgba(0, 212, 255, 0.3), inset 0 0 0 0.5px rgba(255, 255, 255, 0.05);
...
.custom-select.status-filter-select {
  background: var(--blue-team-darker-bg);
  border: 0.5px solid var(--blue-team-border);
...
.header-filter :deep(.el-button) {
  background: linear-gradient(135deg, rgba(0, 212, 255, 0.25) 0%, rgba(0, 212, 255, 0.4) 100%);
...
.header-filter :deep(.el-button:hover) {
  background: linear-gradient(135deg, rgba(0, 212, 255, 0.35) 0%, rgba(0, 212, 255, 0.5) 100%);
  border-color: rgba(0, 212, 255, 0.6);
...
:deep(.el-pager li.is-active) {
  background: var(--blue-team-primary);
  color: white;
