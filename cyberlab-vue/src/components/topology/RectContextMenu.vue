<template>
  ...
  >
    ...
    <div class="menu-item" @click="adjustSize">
      <span class="menu-icon">📐</span>
      <span class="menu-text">调整尺寸</span>
    ...
    <div class="menu-item" @click="togglePin">
      <span class="menu-icon">📌</span>
      <span class="menu-text">{{ rectElement?.pinned ? '取消固定' : '固定位置' }}</span>
      <span class="menu-shortcut">⌘L</span>
    </div>
    
    <div class="menu-divider"></div>
    
    <div class="menu-item destructive" @click="deleteRect">
      <span class="menu-icon">🗑️</span>
      <span class="menu-text">删除矩形</span>
      <span class="menu-shortcut">⌫</span>
    </div>
  </BaseContextMenu>
</template>
...
const editLabel = () => {
  emit('edit-label', props.rectElement)
  emit('close')
}
...
const selectBorderColor = (color) => {
  emit('modify-border-color', props.rectElement, color)
  showBorderColorMenu.value = false
  emit('close')
}

const selectFillColor = (color) => {
  emit('modify-fill-color', props.rectElement, color)
  showFillColorMenu.value = false
  emit('close')
}

const adjustSize = () => {
  emit('adjust-size', props.rectElement)
  emit('close')
}

const togglePin = () => {
  emit('toggle-pin', props.rectElement)
  emit('close')
}

const deleteRect = () => {
  emit('delete-rect', props.rectElement)
  emit('close')
}
</script>
